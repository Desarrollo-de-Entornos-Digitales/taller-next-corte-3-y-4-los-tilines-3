            ...
            <figure className={`h-40 ${config.bg} flex items-center justify-center relative`}>
                ...
                {(item?.image || image.includes('/')) && (
                    <img
                        src={displayImage}
                        alt={displayTitle}
                        className="max-h-full w-full object-cover mix-blend-multiply opacity-90"
                    />
                )}
            </figure>
            <div className="card-body p-5">
                <div className="flex justify-between items-start mb-4">
                    <div className="flex-1 pr-4">
                        <h2 className="text-sm font-bold text-gray-900 mb-1">{displayTitle}</h2>
                ...
                <div className="card-actions justify-start mt-4">
                    <button
                        className={`btn border-none px-6 text-white font-bold rounded-2xl shadow-sm transition-all active:scale-95 text-xs h-10 min-h-0 ${
                            isCompleted ? 'bg-zinc-800 hover:bg-black' : 'bg-[#4A86F7] hover:bg-blue-600'
                        }`}
                        onClick={handleClick}
                    >
                        {displayButtonText}
