...
export default function CreateModulePage() {
    ...
    // Efecto para calcular el level_order automáticamente cuando cambia el curso
    useEffect(() => {
        if (!formData.course_id) return;

        const fetchModulesForCourse = async () => {
            try {
                const modulesForCourse = await moduleService.getByCourseId(formData.course_id);
                const maxLevel =
                    modulesForCourse.length > 0 ? Math.max(...modulesForCourse.map((m) => m.level_order)) : 0;

                const nextLevel = maxLevel + 1;
                setSuggestedLevel(nextLevel);
                setFormData((prev) => ({ ...prev, level_order: nextLevel }));
            } catch (err) {
                console.error('Error al calcular el nivel sugerido', err);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(err);
            setError(err.response?.data?.message || err.message || 'Error al crear el módulo');
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-2">
                                {suggestedLevel
                                    ? `Sugerido automáticamente: Nivel ${suggestedLevel} (puedes cambiarlo si deseas).`
                                    : 'Determina el orden en el que el estudiante debe completar los módulos.'}
                            </p>
                        </div>
