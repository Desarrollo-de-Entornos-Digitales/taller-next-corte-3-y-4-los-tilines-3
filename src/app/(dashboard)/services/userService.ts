...
const normalizeRoleName = (user: RawUser): string => {
    const rawRole =
        user.roleName ?? user.role_name ?? (typeof user.role === 'string' ? user.role : user.role?.name) ?? '';
...
const getProfessorRoleIds = async (): Promise<number[]> => {
    try {
        ...
        return roles.filter((r) => ['profesor', 'professor'].includes(String(r.name).toLowerCase())).map((r) => r.id);
    } catch {
        return [];
    }
};
...
export const getProfessors = async (): Promise<UserBrief[]> => {
    ...
    try {
        ...
        if (source.length === 0 && lastError) {
            const message =
                lastError?.response?.data?.message || lastError?.message || 'Unable to load users from backend';
